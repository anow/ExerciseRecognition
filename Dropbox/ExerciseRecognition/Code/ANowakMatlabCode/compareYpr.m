function [rTrain rTest] = compareYpr(bs)
% Input:
...
for i = 1:length(vars)
    if(strmatch('ae', char(vars{i})))
        a1 = a1+1;
        ae(a1,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
    if(strmatch('ai', char(vars{i})))
        a2 = a2+1;
        ai(a2,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
   if(strmatch('ef', char(vars{i})))
       a3 = a3+1;
        ef(a3,:) = getMeanYpr(newData1.(vars{i}));
        
   end
    
       if(strmatch('fp', char(vars{i})))
           a4 = a4+1;
        fp(a4,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
    if(strmatch('fs', char(vars{i})))
        a5 = a5+1;
        fs(a5,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
   if(strmatch('sa', char(vars{i})))
       a6 = a6+1;
        sa(a6,:) = getMeanYpr(newData1.(vars{i}));
        
   end
    
   if(strmatch('se', char(vars{i})))
       a7 = a7+1;
        se(a7,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
    if(strmatch('sf', char(vars{i})))
        a8 = a8+1;
        sf(a8,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
   if(strmatch('we', char(vars{i})))
       a9 = a9+1;
        we(a9,:) = getMeanYpr(newData1.(vars{i}));
        
   end
   
      if(strmatch('wf', char(vars{i})))
          a10 = a10+1;
        wf(a10,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
    if(strmatch('wr', char(vars{i})))
        a11 = a11+1;
        wr(a11,:) = getMeanYpr(newData1.(vars{i}));
        
    end
    
   if(strmatch('wu', char(vars{i})))
       a12 = a12+1;
        wu(a12,:) = getMeanYpr(newData1.(vars{i}));
