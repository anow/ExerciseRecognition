function [rTrain rTest] = whichCoeffs()
...
vec1 = [zeros(trainSize,1); zeros(trainSize,1); zeros(trainSize,1); zeros(trainSize,1); ones(trainSize,1); ones(trainSize,1); zeros(5,1); ones(trainSize,1); ones(trainSize,1);ones(trainSize,1);ones(trainSize,1);ones(trainSize,1)];

vec1test = [zeros(xAE-trainSize,1); zeros(xAI-trainSize,1); zeros(xEF-trainSize,1);zeros(xFS-trainSize,1); ones(xSA-trainSize,1);ones(xSE-trainSize,1);ones(3,1);ones(xSF-trainSize,1);ones(xWE-trainSize,1);ones(xWF-trainSize,1);ones(xWR-trainSize,1);ones(xWU-trainSize,1)];
...
for i=1:bound
    ...
    rTest(i,1) = 100 -sum(abs(resultTest1-vec1test))/size(resultTest1,1)*100;
    rTrain(i,:) = [b (100 -sum(abs(resultTrain1-vec1))/size(resultTrain1,1)*100)];
end