function [rTrain rTest] = compareAllPosVsTime(timeUpperBound)

% load all upper limb data
newData1 = load('-mat', 'allOldPatients.mat');
vars = fieldnames(newData1);

rTest = zeros(timeUpperBound,3);
rTrain = zeros(timeUpperBound,3);

for jo = 1:timeUpperBound
ae = zeros(50,24);
ai = ae;
ef = ae;
fp = ae;
...
for i = 1:length(vars)
    if(strmatch('ae', char(vars{i})))
        a1 = a1+1;
        ae(a1,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
    if(strmatch('ai', char(vars{i})))
        a2 = a2+1;
        ai(a2,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
   if(strmatch('ef', char(vars{i})))
       a3 = a3+1;
        ef(a3,:) = getNextRows(newData1.(vars{i}),jo); 
   end
    
       if(strmatch('fp', char(vars{i})))
           a4 = a4+1;
        fp(a4,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
    if(strmatch('fs', char(vars{i})))
        a5 = a5+1;
        fs(a5,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
   if(strmatch('sa', char(vars{i})))
       a6 = a6+1;
        sa(a6,:) = getNextRows(newData1.(vars{i}),jo);
   end
    
   if(strmatch('se', char(vars{i})))
       a7 = a7+1;
        se(a7,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
    if(strmatch('sf', char(vars{i})))
        a8 = a8+1;
        sf(a8,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
   if(strmatch('we', char(vars{i})))
       a9 = a9+1;
        we(a9,:) = getNextRows(newData1.(vars{i}),jo);
   end
   
   if(strmatch('wf', char(vars{i})))
          a10 = a10+1;
        wf(a10,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
    if(strmatch('wr', char(vars{i})))
        a11 = a11+1;
        wr(a11,:) = getNextRows(newData1.(vars{i}),jo);
    end
    
   if(strmatch('wu', char(vars{i})))
       a12 = a12+1;
        wu(a12,:) = getNextRows(newData1.(vars{i}),jo);
    end
...
%             accuracy2(k,j)=svmtrain(vec0, train1, str5);
%         end
%     end
...
str5 = ['-b 1 '];
    model5 = svmtrain(vec0, train1, str5);
    resultTrain1 = svmpredict(vec0, train1, model5);
    resultTest1 = svmpredict(vec0test, test1, model5);
    rTest(jo,1) = (100 -sum(abs(resultTest1-vec0test))/size(resultTest1,1)*100);
    rTrain(jo,1) = (100 -sum(abs(resultTrain1-vec0))/size(resultTrain1,1)*100);
...
% distObj2 = [ae(1:trainSize,:); ai(1:trainSize,:); fp(1:5,:);fs(1:trainSize,:);we(1:trainSize,:);wf(1:trainSize,:);wr(1:trainSize,:);wu(1:trainSize,:)];
% k = random('unif',1,7*trainSize+5,4*trainSize,1);
% G = zeros(trainSize, 1);
% for i=1:4*trainSize
%     %floor(k(i,1))
%     G(i) = pdist([distObj1(i,:); distObj2(floor(k(i,1)),:)]);
...
%     [R2,C2] = ind2sub(size(accuracy2),location2);
    ...
    rTest(jo,2) = 100 -sum(abs(resultTest1-vec1test))/size(resultTest1,1)*100;
    rTrain(jo,2) = 100 -sum(abs(resultTrain1-vec1))/size(resultTrain1,1)*100;


% Position 2 svm with parameter lookup
...
% distObj1 = [we(1:trainSize,:);wf(1:trainSize,:);wr(1:trainSize,:);wu(1:trainSize,:)];
% distObj2 = [ae(1:trainSize,:); ai(1:trainSize,:); fp(1:5,:);fs(1:trainSize,:);ef(1:trainSize,:);sa(1:trainSize,:);se(1:trainSize,:);sf(1:trainSize,:)];
% k = random('unif',1,7*trainSize+5,4*trainSize,1);
% G = zeros(trainSize, 1);
% for i=1:4*trainSize
%     %floor(k(i,1))
...
%         for j=1:7
%             str5=['-v 5 -c ' num2str(C2set(k)) ' -g ' num2str(G2set(j))];
%             accuracy2(k,j)=svmtrain(vec2, train1, str5);
%         end
%     end
%     [value2, location2] = max(accuracy2(:));
%     [R2,C2] = ind2sub(size(accuracy2),location2);
    str5 = ['-b 1 '];
    model5 = svmtrain(vec1, train1, str5);
    resultTrain1 = svmpredict(vec2, train1, model5);
    resultTest1 = svmpredict(vec2test, test1, model5);
   
    
    rTest(jo,3) = 100 -sum(abs(resultTest1-vec2test))/size(resultTest1,1)*100;
    rTrain(jo,3) = 100 -sum(abs(resultTrain1-vec2))/size(resultTrain1,1)*100;

end
end

function reduced = getRow(input,rowNum)
    reduced = input(rowNum,3:6);
end
