function [rTest rTrain] = howMuchData(newData1, rowNum)
...
xAE = min([a4 a5 a6]);
trainSize = floor(xAE *2/3);
vec1 = [zeros(trainSize,1); zeros(trainSize,1); ones(trainSize,1)];
train1 = [hfl(1:trainSize,:); kfl(1:trainSize,:); haal(1:trainSize,:)];
vec1test = [zeros(a4-trainSize,1); zeros(a5-trainSize,1); ones(a6-trainSize,1)];
test1 = [hfl((trainSize+1):a4,:); kfl((trainSize+1):a5,:); haal((trainSize+1):a6,:)];
...
train1 = [hfl(1:trainSize,:); kfl(1:trainSize,:)];
vec1test = [ones(a4-trainSize,1); zeros(a5-trainSize,1)];
test1 = [hfl((trainSize+1):a4,:); kfl((trainSize+1):a5,:)];
