function [rTrain rTest] = compareAllPositionsUpperLimb(bs)
% function runs svm to distinguish all 3 upper limb positions with different basis for parameter lookup
% Input:
...
for i = 1:length(vars)
    if(strmatch('ae', char(vars{i})))
        a1 = a1+1;
        ae(a1,:) = getFirst(newData1.(vars{i}));
        
    end
    
    if(strmatch('ai', char(vars{i})))
        a2 = a2+1;
        ai(a2,:) = getFirst(newData1.(vars{i}));
        
    end
    
   if(strmatch('ef', char(vars{i})))
       a3 = a3+1;
        ef(a3,:) = getFirst(newData1.(vars{i}));
        
   end
    
       if(strmatch('fp', char(vars{i})))
           a4 = a4+1;
        fp(a4,:) = getFirst(newData1.(vars{i}));
        
    end
    
    if(strmatch('fs', char(vars{i})))
        a5 = a5+1;
        fs(a5,:) = getFirst(newData1.(vars{i}));
        
    end
    
   if(strmatch('sa', char(vars{i})))
       a6 = a6+1;
        sa(a6,:) = getFirst(newData1.(vars{i}));
        
   end
    
   if(strmatch('se', char(vars{i})))
       a7 = a7+1;
        se(a7,:) = getFirst(newData1.(vars{i}));
        
    end
    
    if(strmatch('sf', char(vars{i})))
        a8 = a8+1;
        sf(a8,:) = getFirst(newData1.(vars{i}));
        
    end
    
   if(strmatch('we', char(vars{i})))
       a9 = a9+1;
        we(a9,:) = getFirst(newData1.(vars{i}));
        
   end
   
      if(strmatch('wf', char(vars{i})))
          a10 = a10+1;
        wf(a10,:) = getFirst(newData1.(vars{i}));
        
    end
    
    if(strmatch('wr', char(vars{i})))
        a11 = a11+1;
        wr(a11,:) = getFirst(newData1.(vars{i}));
        
    end
    
   if(strmatch('wu', char(vars{i})))
       a12 = a12+1;
        wu(a12,:) = getFirst(newData1.(vars{i}));
...
for i=0:bs
    ...
    rTest(i+1,1) = (100 -sum(abs(resultTest1-vec0test))/size(resultTest1,1)*100);
    rTrain(i+1,1) = (100 -sum(abs(resultTrain1-vec0))/size(resultTrain1,1)*100);
end
...
distObj1 = [ef(1:trainSize,:);sa(1:trainSize,:);se(1:trainSize,:)];
distObj2 = [ae(1:trainSize,:); ai(1:trainSize,:); fp(1:5,:);fs(1:trainSize,:);sf(1:trainSize,:);we(1:trainSize,:);wf(1:trainSize,:);wr(1:trainSize,:);wu(1:trainSize,:)];
k = random('unif',1,8*trainSize,4*trainSize,1);
G = zeros(trainSize, 1);
for i=1:4*trainSize
    %floor(k(i,1))
    G(i) = pdist([distObj1(i,:); distObj2(floor(k(i,1)),:)]);
...
for i=0:bs
    ...
    rTest(i+1,3) = 100 -sum(abs(resultTest1-vec2test))/size(resultTest1,1)*100;
    rTrain(i+1,3) = 100 -sum(abs(resultTrain1-vec2))/size(resultTrain1,1)*100;
end
